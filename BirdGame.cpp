...
void BirdGame::init()
{
	...
	for (int i = 0; i < length; i++)
	{
		...
		auto wallPc = obj->addComponent<PhysicsComponent>();
		...
		auto coin = createGameObject();
		obj->name = "Coin";
		auto coinSo = coin->addComponent<SpriteComponent>();
		int coinY = (rand() % int(windowSize.y - coinSprite.getSpriteSize().y - background2Component.getY()) + coinSprite.getSpriteSize().y / 2 + background2Component.getY());
		glm::vec2 coinPos{ i * 300 + xOffset + 150,  coinY };
		coin->setPosition(coinPos);
		coinSo->setSprite(coinSprite);
		auto coinPc = coin->addComponent<PhysicsComponent>();
		coinPc->initCircle(b2_kinematicBody, 10 / physicsScale, coinPos / physicsScale, 1);
		coinPc->setSensor(true);
